...
class ShoppingSearchPreferencesActivity : AppCompatActivity() {
    ...
    private fun setObserver() {
        viewModel.notifyItemMoved.observe(this, Observer {
            adapter.notifyItemMoved(it.fromPosition, it.toPosition)
        })
        viewModel.switchCheckChange.observe(this, Observer {
            viewModel.sitesLiveData.value?.get(it.index)?.data?.toggleOn = it.isChecked
            viewModel.handleAllSwitch()
        })
